...
# Create our web session (link) from Python to the DB, the transaction control
...
def home():
    ...
    return (
        ...
        f"Welcome...you have reached the Hawaiian Weather API!<br/><br/>"
        f"Available routes:<br/><br/>"
        f"View the precipitation from last year: /api/v1.0/precipitation<br/>"
        f"View the temperatures from last year: /api/v1.0/toc<br/>"
        f"View information about the weather stations: /api/v1.0/stations<br/><br/>"
        f"View information about a specific date, add a date: /api/v1.0/<start><br/>"
        f"Example: /api/v1.0/2011-08-11<br/><br/>"
        f"View information about a specific date range, add a start and end date: /api/v1.0/<start>/<end><br/>"
        f"Example: /api/v1.0/2011-08-11/2011-08-12<br/><br/>"
        f"Note: Hawaiian Weather Data is available from: 2010-01-01 to 2017-08-23"
    )
...
def precipitation():
    ...
    yearly_data = session.query(Measurement.date,Measurement.prcp, Measurement.station).\
        filter(and_(Measurement.date >= year_ago, Measurement.date < year_today)).\
        order_by(Measurement.date).all()



    # Convert the query results to a Dictionary using `date` as the key and `toc` as the value.
...
def toc():
    ...
    yearly_data = session.query(Measurement.date,Measurement.toc, Measurement.station).\
        filter(and_(Measurement.date >= year_ago, Measurement.date < year_today)).\
        order_by(Measurement.date).all()
    ...
    for each in yearly_data:
        ...
        yearly_data_dict = {}
        yearly_data_dict['Date'] = each.date
        yearly_data_dict['Temperature'] = each.toc
        yearly_data_dict['Station Name'] = each.station
